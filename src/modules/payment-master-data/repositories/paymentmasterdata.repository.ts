  ...
  export class PaymentMasterDataRepository {
    ...
    client: LoggerClient.getInstance()
      ...
      .get(PaymentMasterDataRepository.name),
        ...
        async findOne(options?: Record<string, any>): Promise<PaymentMasterData | null> {
      ...
      if (isFindOneOptions) {
        logger.info('Ready to findOne (FindOneOptions) PaymentMasterData:', options);
        return this.repository.findOne(options as any);
      }
      const tmp: FindOptionsWhere<PaymentMasterData> = options as FindOptionsWhere<PaymentMasterData>;
