...
export class PaymentMasterDataCommandRepository implements IEventHandler<BaseEvent>{
  ...
  async create(entity: PaymentMasterData): Promise<PaymentMasterData> {
    ...
    if (this.shouldPublishEvent()) {
      this.eventPublisher.publish(new PaymentMasterDataCreatedEvent(result.id, {
        instance: result,
        metadata: {
  ...
  async bulkCreate(entities: PaymentMasterData[]): Promise<PaymentMasterData[]> {
    ...
    if (this.shouldPublishEvent()) {
      this.eventPublisher.publishAll(result.map((el)=>new PaymentMasterDataCreatedEvent(el.id, {
        instance: el,
        metadata: {
  ...
  async update(
    ...
    if(instance && this.shouldPublishEvent()) {
      ...
      this.eventPublisher.publish(new PaymentMasterDataUpdatedEvent(instance.id, {
          instance: instance,
          metadata: {
    ...
    for (const entity of entities) {
      if (entity.id) {
        ...
        if (updatedEntity) {
          ...
          if (this.shouldPublishEvent()) {
            this.eventPublisher.publish(new PaymentMasterDataUpdatedEvent(updatedEntity.id, {
  ...
  async delete(id: string): Promise<DeleteResult> {
     ...
     if (this.shouldPublishEvent()) {
       ...
       this.eventPublisher.publish(new PaymentMasterDataDeletedEvent(id, {
        instance: entity,
        metadata: {
