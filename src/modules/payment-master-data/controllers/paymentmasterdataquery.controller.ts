...
@ApiTags("PaymentMasterData Query")
@Controller("paymentmasterdatas/query")
export class PaymentMasterDataQueryController {
  #logger = new Logger(PaymentMasterDataQueryController.name);

  constructor(private readonly service: PaymentMasterDataQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all paymentmasterdata with optional pagination" })
  @ApiResponse({ status: 200, type: PaymentMasterDatasResponse })
  @ApiQuery({ name: "options", required: false, type: PaymentMasterDataDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<PaymentMasterData>    
  ): Promise<PaymentMasterDatasResponse<PaymentMasterData>> {
    try {
     
      const paymentmasterdatas = await this.service.findAll(options);
      logger.info("Retrieving all paymentmasterdata");
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get paymentmasterdata by ID" })
  @ApiResponse({ status: 200, type: PaymentMasterDataResponse<PaymentMasterData> })
  @ApiResponse({ status: 404, description: "PaymentMasterData not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the paymentmasterdata to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<PaymentMasterDataResponse<PaymentMasterData>> {
    try {
      const paymentmasterdata = await this.service.findOne({ where: { id } });
      if (!paymentmasterdata) {
        throw new NotFoundException(
          "PaymentMasterData no encontrado para el id solicitado"
        );
      }
      return paymentmasterdata;
    } catch (error) {
      logger.error(error);
      return Helper.throwCachedError(error);
    }
  ...
  @ApiOperation({ summary: "Find paymentmasterdata by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter paymentmasterdata', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: PaymentMasterDatasResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
  ...
  async findByField(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "PaymentMasterData no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find paymentmasterdatas with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<PaymentMasterData>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all paymentmasterdatas" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count paymentmasterdatas with conditions" })
  @ApiResponse({ status: 200, type: PaymentMasterDatasResponse<PaymentMasterData> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one paymentmasterdata with conditions" })
  @ApiResponse({ status: 200, type: PaymentMasterDataResponse<PaymentMasterData> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @ApiOperation({ summary: "Find one paymentmasterdata or return error" })
  @ApiResponse({ status: 200, type: PaymentMasterDataResponse<PaymentMasterData> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
