...
@Injectable()
export class PaymentMethodTypeQueryService implements OnModuleInit{
  // Private properties
  readonly #logger = new Logger(PaymentMethodTypeQueryService.name);
  private readonly loggerClient = LoggerClient.getInstance();

  constructor(private readonly repository: PaymentMethodTypeQueryRepository,
  ...
  private validate(): void {
    try {
      ...
      if (!(entityInstance instanceof BaseEntity)) {
        let sms = `El tipo ${PaymentMethodType.name} no extiende de BaseEntity. Asegúrate de que todas las entidades hereden correctamente.`;
        logger.info(sms);
        throw new Error(sms);
      }
    } catch (error) {
  ...
  async findAll(
    ...
    try {
      const paymentmethodtypes = await this.repository.findAll(options);
      // Devolver respuesta
      logger.info("sms");
      return {
        ok: true,
        message: "Listado de paymentmethodtypes obtenido con éxito",
        ...
        count: paymentmethodtypes.length,
      };
    } catch (error) {
      // Imprimir error
      logger.error(error);
  ...
  async findById(id: string): Promise<PaymentMethodTypeResponse<PaymentMethodType>> {
    try {
      const paymentmethodtype = await this.repository.findOne({
        where: { id },
        relations: [],
      });
  ...
  async findByField(
    ...
    try {
      ...
      if (!entities)
        throw new NotFoundException(
          "PaymentMethodTypes no encontrados para la propiedad y valor especificado"
        );
      // Devolver paymentmethodtype
      return {
        ok: true,
        message: "PaymentMethodTypes obtenidos con éxito.",
        data: entities,
        pagination: Helper.getPaginator(
          paginationArgs ? paginationArgs.page : 1,
          paginationArgs ? paginationArgs.size : 25,
  ...
  async findOne(where?: Record<string, any>): Promise<PaymentMethodTypeResponse<PaymentMethodType>> {
    try {
