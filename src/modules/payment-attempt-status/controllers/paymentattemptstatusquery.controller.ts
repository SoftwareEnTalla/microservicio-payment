...
import { PaymentAttemptStatusDto } from "../dtos/all-dto";
...
@ApiTags("PaymentAttemptStatus Query")
@UseGuards(PaymentAttemptStatusAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("paymentattemptstatuss/query")
export class PaymentAttemptStatusQueryController {
  #logger = new Logger(PaymentAttemptStatusQueryController.name);

  constructor(private readonly service: PaymentAttemptStatusQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all paymentattemptstatus with optional pagination" })
  @ApiResponse({ status: 200, type: PaymentAttemptStatussResponse })
  @ApiQuery({ name: "options", required: false, type: PaymentAttemptStatusDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<PaymentAttemptStatus>    
  ): Promise<PaymentAttemptStatussResponse<PaymentAttemptStatus>> {
    try {
     
      const paymentattemptstatuss = await this.service.findAll(options);
      logger.info("Retrieving all paymentattemptstatus");
  ...
  @ApiOperation({ summary: "Find paymentattemptstatus by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter paymentattemptstatus', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: PaymentAttemptStatussResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "PaymentAttemptStatus no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find paymentattemptstatuss with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<PaymentAttemptStatus>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all paymentattemptstatuss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count paymentattemptstatuss with conditions" })
  @ApiResponse({ status: 200, type: PaymentAttemptStatussResponse<PaymentAttemptStatus> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades PaymentAttemptStatuss no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one paymentattemptstatus with conditions" })
  ...
  @ApiOperation({ summary: "Find one paymentattemptstatus or return error" })
  @ApiResponse({ status: 200, type: PaymentAttemptStatusResponse<PaymentAttemptStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad PaymentAttemptStatus no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get paymentattemptstatus by ID" })
  @ApiResponse({ status: 200, type: PaymentAttemptStatusResponse<PaymentAttemptStatus> })
  @ApiResponse({ status: 404, description: "PaymentAttemptStatus not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the paymentattemptstatus to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<PaymentAttemptStatusResponse<PaymentAttemptStatus>> {
    try {
      const paymentattemptstatus = await this.service.findOne({ where: { id } });
      if (!paymentattemptstatus) {
        throw new NotFoundException(
          "PaymentAttemptStatus no encontrado para el id solicitado"
        );
      }
