...
@Injectable()
export class PaymentGatewayStatusService {
  constructor(private readonly repository: PaymentGatewayStatusRepository) {}

  // Métodos delegados
  async findAll(options?: any): Promise<PaymentGatewayStatus[]> {
  ...
  async findByField(field: string, value: any, page: number, limit: number): Promise<PaymentGatewayStatus[]> {
    return this.repository.findByField(field, value, page, limit);
  }

  async findWithPagination(options: any, page: number, limit: number): Promise<PaymentGatewayStatus[]> {
  ...
  async findOne(where?: Record<string, any>): Promise<PaymentGatewayStatus | null> {
  ...
  async update(id: string, partialEntity: Partial<PaymentGatewayStatus>): Promise<PaymentGatewayStatus | null> {
