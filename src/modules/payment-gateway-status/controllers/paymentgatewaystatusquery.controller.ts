...
@ApiTags("PaymentGatewayStatus Query")
@UseGuards(PaymentGatewayStatusAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("paymentgatewaystatuss/query")
export class PaymentGatewayStatusQueryController {
  #logger = new Logger(PaymentGatewayStatusQueryController.name);

  constructor(private readonly service: PaymentGatewayStatusQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all paymentgatewaystatus with optional pagination" })
  @ApiResponse({ status: 200, type: PaymentGatewayStatussResponse })
  @ApiQuery({ name: "options", required: false, type: PaymentGatewayStatusDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<PaymentGatewayStatus>    
  ): Promise<PaymentGatewayStatussResponse<PaymentGatewayStatus>> {
    try {
     
      const paymentgatewaystatuss = await this.service.findAll(options);
      logger.info("Retrieving all paymentgatewaystatus");
  ...
  @ApiOperation({ summary: "Find paymentgatewaystatus by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter paymentgatewaystatus', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: PaymentGatewayStatussResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "PaymentGatewayStatus no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find paymentgatewaystatuss with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<PaymentGatewayStatus>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all paymentgatewaystatuss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count paymentgatewaystatuss with conditions" })
  @ApiResponse({ status: 200, type: PaymentGatewayStatussResponse<PaymentGatewayStatus> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one paymentgatewaystatus with conditions" })
  ...
  @ApiOperation({ summary: "Find one paymentgatewaystatus or return error" })
  @ApiResponse({ status: 200, type: PaymentGatewayStatusResponse<PaymentGatewayStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get paymentgatewaystatus by ID" })
  @ApiResponse({ status: 200, type: PaymentGatewayStatusResponse<PaymentGatewayStatus> })
  @ApiResponse({ status: 404, description: "PaymentGatewayStatus not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the paymentgatewaystatus to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<PaymentGatewayStatusResponse<PaymentGatewayStatus>> {
    try {
      const paymentgatewaystatus = await this.service.findOne({ where: { id } });
