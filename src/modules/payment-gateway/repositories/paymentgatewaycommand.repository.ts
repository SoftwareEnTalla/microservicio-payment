...
export class PaymentGatewayCommandRepository implements IEventHandler<BaseEvent>{
  ...
  async create(entity: PaymentGateway): Promise<PaymentGateway> {
    ...
    if (this.shouldPublishEvent()) {
      this.eventPublisher.publish(new PaymentGatewayCreatedEvent(result.id, {
        instance: result,
        metadata: {
  ...
  async bulkCreate(entities: PaymentGateway[]): Promise<PaymentGateway[]> {
    ...
    if (this.shouldPublishEvent()) {
      this.eventPublisher.publishAll(result.map((el)=>new PaymentGatewayCreatedEvent(el.id, {
        instance: el,
        metadata: {
  ...
  async update(
    ...
    if(instance && this.shouldPublishEvent()) {
      ...
      this.eventPublisher.publish(new PaymentGatewayUpdatedEvent(instance.id, {
          instance: instance,
          metadata: {
    ...
    for (const entity of entities) {
      if (entity.id) {
        ...
        if (updatedEntity) {
          ...
          if (this.shouldPublishEvent()) {
            this.eventPublisher.publish(new PaymentGatewayUpdatedEvent(updatedEntity.id, {
  ...
  async delete(id: string): Promise<DeleteResult> {
     ...
     if (this.shouldPublishEvent()) {
       ...
       this.eventPublisher.publish(new PaymentGatewayDeletedEvent(id, {
        instance: entity,
        metadata: {
