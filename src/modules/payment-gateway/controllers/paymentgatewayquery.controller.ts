...
@ApiTags("PaymentGateway Query")
@Controller("paymentgateways/query")
export class PaymentGatewayQueryController {
  #logger = new Logger(PaymentGatewayQueryController.name);

  constructor(private readonly service: PaymentGatewayQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all paymentgateway with optional pagination" })
  @ApiResponse({ status: 200, type: PaymentGatewaysResponse })
  @ApiQuery({ name: "options", required: false, type: PaymentGatewayDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<PaymentGateway>    
  ): Promise<PaymentGatewaysResponse<PaymentGateway>> {
    try {
     
      const paymentgateways = await this.service.findAll(options);
      logger.info("Retrieving all paymentgateway");
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get paymentgateway by ID" })
  @ApiResponse({ status: 200, type: PaymentGatewayResponse<PaymentGateway> })
  @ApiResponse({ status: 404, description: "PaymentGateway not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the paymentgateway to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<PaymentGatewayResponse<PaymentGateway>> {
    try {
      const paymentgateway = await this.service.findOne({ where: { id } });
      if (!paymentgateway) {
  ...
  @ApiOperation({ summary: "Find paymentgateway by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter paymentgateway', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: PaymentGatewaysResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
  ...
  async findByField(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "PaymentGateway no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find paymentgateways with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<PaymentGateway>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all paymentgateways" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count paymentgateways with conditions" })
  @ApiResponse({ status: 200, type: PaymentGatewaysResponse<PaymentGateway> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one paymentgateway with conditions" })
  @ApiResponse({ status: 200, type: PaymentGatewayResponse<PaymentGateway> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @ApiOperation({ summary: "Find one paymentgateway or return error" })
  @ApiResponse({ status: 200, type: PaymentGatewayResponse<PaymentGateway> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
