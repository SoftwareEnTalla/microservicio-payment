...
@Injectable()
export class PaymentMerchantGatewayEligibilityUpdatedSyncSaga {
  private readonly logger = new Logger(PaymentMerchantGatewayEligibilityUpdatedSyncSaga.name);
  ...
  // Actualiza la proyección local de elegibilidad cuando el merchant cambia la configuración operativa de la pasarela.
  ...
  onMerchantGatewayConfigUpdated = ($events: Observable<MerchantGatewayConfigUpdatedEvent>) => {
    return $events.pipe(
      ...
      tap(event => {
        this.logger.log(`Saga payment-merchant-gateway-eligibility-updated-sync recibió MerchantGatewayConfigUpdated: ${event.aggregateId}`);
  ...
  private buildCommandMetadata(event: any, correlationId: string) {
    ...
    return {
      ...
      saga: 'payment-merchant-gateway-eligibility-updated-sync',
    };
  }
