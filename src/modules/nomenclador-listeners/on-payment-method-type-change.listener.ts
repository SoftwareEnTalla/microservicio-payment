 ...
 * Licencia: MIT
 *
 * OnPaymentMethodTypeChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `payment-method-type` (owner: payment). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'payment-method-type';
...
const TOPICS: string[] = [
  'payment-method-type-created',
  'payment-method-type-updated',
  'payment-method-type-deleted',
];

@Injectable()
export class OnPaymentMethodTypeChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnPaymentMethodTypeChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnPaymentMethodTypeChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnPaymentMethodTypeChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onPaymentMethodTypeChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
