 ...
 * Licencia: MIT
 *
 * OnSettlementModeChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `settlement-mode` (owner: catalog). Suscribe a EventBus local y a Kafka.
 *
...
const TOPICS: string[] = [
  'settlement-mode-created',
  'settlement-mode-updated',
  'settlement-mode-deleted',
];

@Injectable()
export class OnSettlementModeChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnSettlementModeChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnSettlementModeChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnSettlementModeChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onSettlementModeChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
