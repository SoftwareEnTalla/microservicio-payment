 ...
 * Licencia: MIT
 *
 * OnPaymentMerchantGatewayEligibilityStatusChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `payment-merchant-gateway-eligibility-status` (owner: payment). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'payment-merchant-gateway-eligibility-status';
const OWNER_BC = 'payment';
...
const TOPICS: string[] = [
  'payment-merchant-gateway-eligibility-status-created',
  'payment-merchant-gateway-eligibility-status-updated',
  'payment-merchant-gateway-eligibility-status-deleted',
];

@Injectable()
export class OnPaymentMerchantGatewayEligibilityStatusChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnPaymentMerchantGatewayEligibilityStatusChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnPaymentMerchantGatewayEligibilityStatusChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnPaymentMerchantGatewayEligibilityStatusChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onPaymentMerchantGatewayEligibilityStatusChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
