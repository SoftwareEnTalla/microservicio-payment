 ...
 * Licencia: MIT
 *
 * OnPaymentAttemptStatusChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `payment-attempt-status` (owner: payment). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'payment-attempt-status';
...
const TOPICS: string[] = [
  'payment-attempt-status-created',
  'payment-attempt-status-updated',
  'payment-attempt-status-deleted',
];

@Injectable()
export class OnPaymentAttemptStatusChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnPaymentAttemptStatusChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnPaymentAttemptStatusChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnPaymentAttemptStatusChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onPaymentAttemptStatusChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
