 ...
 * Licencia: MIT
 *
 * OnCustomerOnboardingStatusChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `customer-onboarding-status` (owner: customer). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'customer-onboarding-status';
const OWNER_BC = 'customer';
...
const TOPICS: string[] = [
  'customer-onboarding-status-created',
  'customer-onboarding-status-updated',
  'customer-onboarding-status-deleted',
];

@Injectable()
export class OnCustomerOnboardingStatusChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnCustomerOnboardingStatusChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnCustomerOnboardingStatusChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnCustomerOnboardingStatusChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onCustomerOnboardingStatusChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
