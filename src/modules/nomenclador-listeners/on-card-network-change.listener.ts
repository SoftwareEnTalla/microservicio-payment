 ...
 * Licencia: MIT
 *
 * OnCardNetworkChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `card-network` (owner: payment). Suscribe a EventBus local y a Kafka.
 *
...
const TOPICS: string[] = [
  'card-network-created',
  'card-network-updated',
  'card-network-deleted',
];

@Injectable()
export class OnCardNetworkChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnCardNetworkChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnCardNetworkChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnCardNetworkChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onCardNetworkChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
