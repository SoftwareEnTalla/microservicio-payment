...
export class PaymentAttemptCommandRepository implements IEventHandler<BaseEvent>{
  ...
  async create(entity: PaymentAttempt): Promise<PaymentAttempt> {
    ...
    if (this.shouldPublishEvent()) {
      const __dualEvt1 = new PaymentAttemptCreatedEvent(result.id, {
        instance: result,
        metadata: {
  ...
  async bulkCreate(entities: PaymentAttempt[]): Promise<PaymentAttempt[]> {
    ...
    if (this.shouldPublishEvent()) {
      const __dualEvts2 = result.map((el)=>new PaymentAttemptCreatedEvent(el.id, {
        instance: el,
        metadata: {
  ...
  async update(
    ...
    if(instance && this.shouldPublishEvent()) {
      ...
      const __dualEvt3 = new PaymentAttemptUpdatedEvent(instance.id, {
          instance: instance,
          metadata: {
    ...
    for (const entity of entities) {
      if (entity.id) {
        ...
        if (updatedEntity) {
          ...
          if (this.shouldPublishEvent()) {
            const __dualEvt4 = new PaymentAttemptUpdatedEvent(updatedEntity.id, {
  ...
  async delete(id: string): Promise<DeleteResult> {
     ...
     if (this.shouldPublishEvent()) {
       ...
       const __dualEvt5 = new PaymentAttemptDeletedEvent(id, {
        instance: entity,
        metadata: {
