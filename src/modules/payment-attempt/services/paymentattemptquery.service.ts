...
@Injectable()
export class PaymentAttemptQueryService implements OnModuleInit{
  // Private properties
  readonly #logger = new Logger(PaymentAttemptQueryService.name);
  private readonly loggerClient = LoggerClient.getInstance();

  constructor(private readonly repository: PaymentAttemptQueryRepository,
  ...
  private validate(): void {
    try {
      ...
      if (!(entityInstance instanceof BaseEntity)) {
        let sms = `El tipo ${PaymentAttempt.name} no extiende de BaseEntity. Asegúrate de que todas las entidades hereden correctamente.`;
        logger.info(sms);
        throw new Error(sms);
      }
    } catch (error) {
  ...
  async findAll(
    ...
    try {
      const paymentattempts = await this.repository.findAll(options);
      // Devolver respuesta
      logger.info("sms");
      return {
        ok: true,
        message: "Listado de paymentattempts obtenido con éxito",
  ...
  async findById(id: string): Promise<PaymentAttemptResponse<PaymentAttempt>> {
    try {
      const paymentattempt = await this.repository.findOne({
        where: { id },
        relations: [],
      });
  ...
  async findByField(
    ...
    try {
      ...
      if (!entities)
        throw new NotFoundException(
          "PaymentAttempts no encontrados para la propiedad y valor especificado"
        );
      // Devolver paymentattempt
      return {
        ok: true,
        message: "PaymentAttempts obtenidos con éxito.",
        data: entities,
        pagination: Helper.getPaginator(
          paginationArgs ? paginationArgs.page : 1,
          paginationArgs ? paginationArgs.size : 25,
  ...
  async findOne(where?: Record<string, any>): Promise<PaymentAttemptResponse<PaymentAttempt>> {
    try {
