...
export class PaymentAttemptService {
  ...
  // Métodos delegados
  async findAll(options?: any): Promise<PaymentAttempt[]> {
  ...
  async findByField(field: string, value: any, page: number, limit: number): Promise<PaymentAttempt[]> {
    return this.repository.findByField(field, value, page, limit);
  }

  async findWithPagination(options: any, page: number, limit: number): Promise<PaymentAttempt[]> {
  ...
  async update(id: string, partialEntity: Partial<PaymentAttempt>): Promise<PaymentAttempt | null> {
