...
@ApiTags("PaymentMerchantGatewayEligibilityStatus Query")
@UseGuards(PaymentMerchantGatewayEligibilityStatusAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("paymentmerchantgatewayeligibilitystatuss/query")
export class PaymentMerchantGatewayEligibilityStatusQueryController {
  #logger = new Logger(PaymentMerchantGatewayEligibilityStatusQueryController.name);

  constructor(private readonly service: PaymentMerchantGatewayEligibilityStatusQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all paymentmerchantgatewayeligibilitystatus with optional pagination" })
  @ApiResponse({ status: 200, type: PaymentMerchantGatewayEligibilityStatussResponse })
  @ApiQuery({ name: "options", required: false, type: PaymentMerchantGatewayEligibilityStatusDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<PaymentMerchantGatewayEligibilityStatus>    
  ): Promise<PaymentMerchantGatewayEligibilityStatussResponse<PaymentMerchantGatewayEligibilityStatus>> {
    try {
     
      const paymentmerchantgatewayeligibilitystatuss = await this.service.findAll(options);
      logger.info("Retrieving all paymentmerchantgatewayeligibilitystatus");
  ...
  @ApiOperation({ summary: "Find paymentmerchantgatewayeligibilitystatus by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter paymentmerchantgatewayeligibilitystatus', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: PaymentMerchantGatewayEligibilityStatussResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "PaymentMerchantGatewayEligibilityStatus no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find paymentmerchantgatewayeligibilitystatuss with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<PaymentMerchantGatewayEligibilityStatus>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all paymentmerchantgatewayeligibilitystatuss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count paymentmerchantgatewayeligibilitystatuss with conditions" })
  @ApiResponse({ status: 200, type: PaymentMerchantGatewayEligibilityStatussResponse<PaymentMerchantGatewayEligibilityStatus> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades PaymentMerchantGatewayEligibilityStatuss no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one paymentmerchantgatewayeligibilitystatus with conditions" })
  ...
  @ApiOperation({ summary: "Find one paymentmerchantgatewayeligibilitystatus or return error" })
  @ApiResponse({ status: 200, type: PaymentMerchantGatewayEligibilityStatusResponse<PaymentMerchantGatewayEligibilityStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad PaymentMerchantGatewayEligibilityStatus no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get paymentmerchantgatewayeligibilitystatus by ID" })
  @ApiResponse({ status: 200, type: PaymentMerchantGatewayEligibilityStatusResponse<PaymentMerchantGatewayEligibilityStatus> })
  @ApiResponse({ status: 404, description: "PaymentMerchantGatewayEligibilityStatus not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the paymentmerchantgatewayeligibilitystatus to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<PaymentMerchantGatewayEligibilityStatusResponse<PaymentMerchantGatewayEligibilityStatus>> {
    try {
      const paymentmerchantgatewayeligibilitystatus = await this.service.findOne({ where: { id } });
      if (!paymentmerchantgatewayeligibilitystatus) {
        throw new NotFoundException(
          "PaymentMerchantGatewayEligibilityStatus no encontrado para el id solicitado"
        );
      }
      return paymentmerchantgatewayeligibilitystatus;
    } catch (error) {
      logger.error(error);
      return Helper.throwCachedError(error);
    }
