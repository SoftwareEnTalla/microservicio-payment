...
export class PaymentCommandRepository implements IEventHandler<BaseEvent>{
  ...
  async create(entity: Payment): Promise<Payment> {
    ...
    if (this.shouldPublishEvent()) {
      const __dualEvt1 = new PaymentCreatedEvent(result.id, {
        instance: result,
        metadata: {
  ...
  async bulkCreate(entities: Payment[]): Promise<Payment[]> {
    ...
    if (this.shouldPublishEvent()) {
      const __dualEvts2 = result.map((el)=>new PaymentCreatedEvent(el.id, {
        instance: el,
        metadata: {
  ...
  async update(
    ...
    if(instance && this.shouldPublishEvent()) {
      ...
      const __dualEvt3 = new PaymentUpdatedEvent(instance.id, {
          instance: instance,
          metadata: {
    ...
    for (const entity of entities) {
      if (entity.id) {
        ...
        if (updatedEntity) {
          ...
          if (this.shouldPublishEvent()) {
            const __dualEvt4 = new PaymentUpdatedEvent(updatedEntity.id, {
  ...
  async delete(id: string): Promise<DeleteResult> {
     ...
     if (this.shouldPublishEvent()) {
       ...
       const __dualEvt5 = new PaymentDeletedEvent(id, {
        instance: entity,
        metadata: {
