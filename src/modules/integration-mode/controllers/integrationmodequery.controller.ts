...
import { IntegrationModeDto } from "../dtos/all-dto";
...
@ApiTags("IntegrationMode Query")
@UseGuards(IntegrationModeAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("integrationmodes/query")
export class IntegrationModeQueryController {
  #logger = new Logger(IntegrationModeQueryController.name);

  constructor(private readonly service: IntegrationModeQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all integrationmode with optional pagination" })
  @ApiResponse({ status: 200, type: IntegrationModesResponse })
  @ApiQuery({ name: "options", required: false, type: IntegrationModeDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<IntegrationMode>    
  ): Promise<IntegrationModesResponse<IntegrationMode>> {
    try {
     
      const integrationmodes = await this.service.findAll(options);
      logger.info("Retrieving all integrationmode");
  ...
  @ApiOperation({ summary: "Find integrationmode by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter integrationmode', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: IntegrationModesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "IntegrationMode no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find integrationmodes with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<IntegrationMode>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all integrationmodes" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count integrationmodes with conditions" })
  @ApiResponse({ status: 200, type: IntegrationModesResponse<IntegrationMode> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one integrationmode with conditions" })
  ...
  @ApiOperation({ summary: "Find one integrationmode or return error" })
  @ApiResponse({ status: 200, type: IntegrationModeResponse<IntegrationMode> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get integrationmode by ID" })
  @ApiResponse({ status: 200, type: IntegrationModeResponse<IntegrationMode> })
  @ApiResponse({ status: 404, description: "IntegrationMode not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the integrationmode to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<IntegrationModeResponse<IntegrationMode>> {
    try {
      const integrationmode = await this.service.findOne({ where: { id } });
      if (!integrationmode) {
        throw new NotFoundException(
