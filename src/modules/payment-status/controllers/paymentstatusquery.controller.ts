...
import { PaymentStatusDto } from "../dtos/all-dto";
...
@ApiTags("PaymentStatus Query")
@UseGuards(PaymentStatusAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("paymentstatuss/query")
export class PaymentStatusQueryController {
  #logger = new Logger(PaymentStatusQueryController.name);

  constructor(private readonly service: PaymentStatusQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all paymentstatus with optional pagination" })
  @ApiResponse({ status: 200, type: PaymentStatussResponse })
  @ApiQuery({ name: "options", required: false, type: PaymentStatusDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<PaymentStatus>    
  ): Promise<PaymentStatussResponse<PaymentStatus>> {
    try {
     
      const paymentstatuss = await this.service.findAll(options);
      logger.info("Retrieving all paymentstatus");
  ...
  @ApiOperation({ summary: "Find paymentstatus by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter paymentstatus', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: PaymentStatussResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "PaymentStatus no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find paymentstatuss with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<PaymentStatus>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all paymentstatuss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count paymentstatuss with conditions" })
  @ApiResponse({ status: 200, type: PaymentStatussResponse<PaymentStatus> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one paymentstatus with conditions" })
  ...
  @ApiOperation({ summary: "Find one paymentstatus or return error" })
  @ApiResponse({ status: 200, type: PaymentStatusResponse<PaymentStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get paymentstatus by ID" })
  @ApiResponse({ status: 200, type: PaymentStatusResponse<PaymentStatus> })
  @ApiResponse({ status: 404, description: "PaymentStatus not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the paymentstatus to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<PaymentStatusResponse<PaymentStatus>> {
    try {
      const paymentstatus = await this.service.findOne({ where: { id } });
      if (!paymentstatus) {
        throw new NotFoundException(
